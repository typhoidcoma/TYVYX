"""
TEKY WiFi Drone Controller - Advanced Edition with Flight Controls
WARNING: Flight control commands are experimental and based on speculation
"""
...
class FlightController:
    ...
    def _send_flight_command(self):
        ...
        Format 1: [CMD_ID, THROTTLE, YAW, PITCH, ROLL, CHECKSUM]
        Format 2: [CMD_ID, ROLL, PITCH, THROTTLE, YAW]
        Format 3: [0x50, THROTTLE_L, THROTTLE_H, YAW, PITCH, ROLL]
        
        We'll try Format 1 first with command ID 0x50 (common for RC control)
        """
        ...
        command = bytes([cmd_id, self.throttle, self.yaw, self.pitch, self.roll, checksum])
        ...
        while True:
            ...
            if video_enabled:
                ...
                if ret and frame is not None:
                    # Add overlay
                    y_offset = 30
                    cv2.putText(frame, "TEKY Drone Controller - Press Q to quit", 
                               (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.6, (0, 255, 0), 2)
                    
                    y_offset += 25
                    cv2.putText(frame, f"Device Type: {drone.device_type} | Connected: {drone.is_connected}", 
                               (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.5, (0, 255, 0), 1)
                    ...
                    cv2.putText(frame, f"Flight Controller: {status_text}", 
                               (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.5, status_color, 2)
            ...
            # Camera controls
                ...
                drone.send_command(drone.CMD_SCREEN_MODE_1 if screen_mode == 1 else drone.CMD_SCREEN_MODE_2, verbose=True)
