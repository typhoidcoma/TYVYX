...
class TEKYDroneController:
    ...
    def _parse_response(self, data: bytes):
        ...
        if len(data) >= 2:
            # Byte 1 is camera switch reset state
            camera_state = data[1]
            
        if len(data) >= 3:
            # Byte 2 is screen switch state
            screen_state = data[2]
        ...
        while True:
            ...
            if video_enabled:
                ...
                if ret and frame is not None:
                    # Add overlay text
                    cv2.putText(frame, "TEKY Drone - Press Q to quit", 
                               (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.7, (0, 255, 0), 2)
                    cv2.putText(frame, f"Device Type: {drone.device_type}", 
                               (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.5, (0, 255, 0), 1)
                    cv2.putText(frame, f"Connected: {drone.is_connected}", 
